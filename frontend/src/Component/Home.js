...
export default function Home() {
    ...
    // ---------------------------
    // STOP CAMERA + STOP LIVE LOOP (Fixed - no dependencies)
    // ---------------------------
    const stopCamera = useCallback(() => {
        // Use refs to avoid dependency on cameraStream state
    ...
    // ---------------------------
    // START CAMERA (Improved with better error handling)
    // ---------------------------
    const startCamera = async () => {
        try {
            setError("");
            ...
            // Use a more reliable way to set up video
            if (videoRef.current) {
                videoRef.current.srcObject = stream;
    ...
    const startLiveDetection = () => {
        if (!cameraReady || !videoRef.current || !canvasRef.current) {
            setError("Camera not ready for live detection.");
            return;
        }
